...
> **Approach (short version).** A config-driven pipeline streams the WorldPop
...
future labour-force growth. That is exactly the kind of signal this dashboard
surfaces per district. *(Totals align with published WorldPop 2025 magnitudes.)*
...
The raw rasters (`data/raw/`) are never committed; they are rebuilt from the
...
1. **Push the repo to GitHub**, including `data/processed/` and `requirements.txt`.
   (Rebuild the data first with `make pipeline` if it is stale.)
2. Go to **[share.streamlit.io](https://share.streamlit.io)**, choose **New app**,
   and select the repo/branch.
...
1. **Catalog** the rasters: enumerate every `(country, sex, age-band)` file from
...
2. **Fetch**: stream each GeoTIFF and the GADM L2 GeoJSON from their public
   URLs. Cached by size; atomic writes; re-runs skip what is already local.
...
| File | What it is |
|------|------------|
| `population_districts.parquet` | Long tidy table, one row per country x district x age-band x sex |
...
**Dashboard** (`streamlit run app/dashboard.py`) provides filters for **country**,
...
- A **population pyramid** (age by sex)
...
- The WorldPop directory holds **60 GeoTIFFs per country** (20 age bands by 3
  ...
  **not** the brief's illustrative `M_0_4.tif`. The age token is a zero-padded
  lower bound (`00, 01, 05, ... 90`); `00` is age under 1 and `90` is the open
  90+ band.
- Under `1km_ua/` only a **`constrained/`** sub-folder is actually published
  ...
  contradictory), so that is the directory used.
...
A naive app would read rasters on every user interaction. That is slow and does
not scale. The key insight is that **the expensive work (raster to district
numbers) is fixed**: it does not depend on what the user filters. So the
architecture splits cleanly into two stages:

```
   heavy, run-once:   rasters  ->  zonal aggregation  ->  tidy parquet
   light, run-often:  tidy parquet  ->  dashboard (reads & filters)
```
...
approach, looping `rasterstats`/`exactextract` per polygon per file, would
re-mask geometry **60 times per country**. But every age/sex raster for a country
shares an **identical grid**. So instead:

1. **Burn the district polygons to that grid once** into an integer label array.
2. Reduce each raster with a single vectorised `np.bincount(labels, weights)`.

That turns *N polygons by M files* of masking into *one pass per file*, O(pixels)
...
The pipeline emits a **long, tidy table** (one row per country x district x
age-band x sex) rather than a wide matrix. Tidy data is what filtering,
...
Resilience here was not theoretical. The first real end-to-end run **failed** on
...
- a **whole-file retry** with backoff that discards partials (covers mid-stream
  disconnects the adapter cannot resume), and
...
Raw counts are not the deliverable; *interpretation* is. So `indicators.py`
...
|----------|--------|-----------------------------|
| Raster work | Pre-compute in pipeline | Dashboard stays instant; vs reading rasters live (slow, does not scale) |
| Zonal method | Rasterize-once + `bincount` | O(pixels); vs per-polygon masking x60 (wasteful) |
...
<div align="center"><sub>A <a href="https://codewitheugene.top/">CodeWithEugene</a> Creation</sub></div>
